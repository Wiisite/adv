...
export default function FrequenciasPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newFrequencia: Frequencia = {
      id: editingFrequencia?.id || Date.now().toString(),
      description: formData.description,
      created_at: editingFrequencia?.created_at || new Date().toISOString(),
    };

    if (editingFrequencia) {
      setFrequencias(frequencias.map(f => f.id === editingFrequencia.id ? newFrequencia : f));
    } else {
      setFrequencias([...frequencias, newFrequencia]);
    }
  ...
  const handleEdit = (frequencia: Frequencia) => {
    ...
    setFormData({ description: frequencia.description });
    setShowModal(true);
  };

  const handleDelete = (id: string) => {
    if (confirm('Deseja realmente excluir esta frequência?')) {
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Frequências</h1>
            <p className="text-gray-600">Gerencie as frequências de pagamento</p>
          </div>
          <button
            onClick={() => {
              resetForm();
              ...
              <tbody className="divide-y divide-gray-200">
                {frequencias.length === 0 ? (
                  ...
                  frequencias.map((frequencia) => (
                    <tr key={frequencia.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 text-sm text-gray-900">{frequencia.description}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <button
                            onClick={() => handleDelete(frequencia.id)}
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div className="bg-cyan-600 text-white px-6 py-4 flex justify-between items-center">
              <h2 className="text-xl font-semibold">
                {editingFrequencia ? 'Editar Frequência' : 'Nova Frequência'}
              </h2>
              <button
                onClick={() => {
                  setShowModal(false);
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Mensal, Trimestral, Anual, etc"
                />
              </div>
